<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        @csrf
            ...
            <x-text-input id="name" class="hidden" type="text" name="careerfair_id" value="{{ $aocf->id }}" />
        ...
        <div class="flex items-center justify-between mt-4">
            <div class="justify-self-start"> 
                <p class="inline-flex text-sm text-gray-600 hover:text-gray-900" href="{{ route('login') }}">
                    {{ __('Already have account?') }}
                </p>
                <a class="ml-1 text-sm text-blue-600 hover:text-blue-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" href="{{ route('login') }}">
                    {{ __('Login') }}
                </a>
            </div>
