...
@section('title')
  <title>Update Notification | DPKKA - Universitas Airlangga</title>
@endsection
...
@section('main')
  <main id="main" class="main">
    ...
    <section class="section">
      <div class="card">
        <div class="card-body">
          <h5 class="card-title">Update Notification</h5>
          <!-- General Form Elements -->
          <form action="{{ route('company-notification-update',$notif->id) }}" method="POST">
            @csrf
            <div class="row mb-3">
              <label for="title" class="col-sm-2 col-form-label">Judul Notifikasi</label>
              <div class="col-sm-10">
                <input type="text" class="form-control" name="title" id="title" value="{{ $notif->title }}">
                @error('title')
            ...
            <div class="row mb-3">
              ...
              <div class="col-sm-10">
                <div>
                  <textarea class="form-control" id="editor" name="message">{{ $notif->message }}</textarea>
                  @error('message')
                    <p class="text-danger">{{ $message }}</p>
                  @enderror
                </div>  
            ...
            <div class="row mb-3">
              ...
              <div class="col-sm-10">
                <select class="form-select" aria-label="Default select example" name="user">
                  ...
                  <option value="Proceed" {{ $user = "Proceed" ? 'selected' : '' ; }}>Proceed Application</option>
                </select>
                @error('user')
                  <p class="text-danger">{{ $message }}</p>
                @enderror
